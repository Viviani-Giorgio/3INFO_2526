...
app.post('/aggiungi', function(req,res){
    ...
    if (req.body.eta_studente > 19 && req.body.eta_studente < 31) {
        console.log(`OK: età ${req.body.eta_studente} valido`);
    } else{
        console.log(`ERROR: età ${req.body.eta_studente} non valido`);
    }
    
    /*res.send(`<script>setTimeout(function(){ window.location.href = "/"; }, 3000);</script>
                    <h1>ERROR: non posso aggiungere lo studente id [${req.body.id_studente}]</h1><br>
                    <script>setTimeout(function(){ window.location.href = "/"; }, 3000);</script>
                    <h1>ERROR: non posso aggiungere lo studente Anni [${req.body.eta_studente}]</h1>`);*/

    //res.send(`<h1>OK studente aggiunto con id [${req.body.id_studente}]</h1><br><h1>OK studente aggiunto con id [${req.body.eta_studente}]</h1>`);
});
